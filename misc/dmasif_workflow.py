...
os.makedirs('misc/dmasif', exist_ok=True)
...
def get_union_iface_res(apo_ref, bound_ref=None, bound_chain=None, bound_ligand=None,
                        pesto_threshold=0.5, work_dir='misc/dmasif/union_tmp',
                        pesto_type=None):
...
def load_or_build_A_fingerprints(pdb_parser, apo_ref, bound_ref=None,
    ...
    if iface_residues is not None:
        ...
        suffix = ''
    cache = f'misc/dmasif_cache/A_interface_cache_{apo_stem}_{bound_stem}{suffix}.npz'
...
def screen_candidates(candidates, A_fps, A_iface_coords, B_pos, B_vdw, pdb_parser):
    ...
    for c_path in candidates:
        ...
        out_pdb = f"misc/dmasif/structures/{name}_docked.pdb"
...
def print_summary(results, tsv_path='misc/dmasif/rankings.tsv'):
    if not results:
        return
...
def main():
    ...
    ap.add_argument('--rankings-tsv', default=None,
                    help='Output TSV path for rankings. Defaults to '
                         'misc/dmasif/rankings_union.tsv (--union) or '
                         'misc/dmasif/rankings.tsv (default).')
    ...
    if args.rankings_tsv is None:
        if iface_residues is not None:
            args.rankings_tsv = f'misc/dmasif/rankings_{apo_stem}_explicit.tsv'
        elif args.union:
            args.rankings_tsv = f'misc/dmasif/rankings_{apo_stem}_union.tsv'
        else:
            args.rankings_tsv = 'misc/dmasif/rankings.tsv'
    ...
    print(f"Docked PDB files saved in: misc/dmasif/structures/")
